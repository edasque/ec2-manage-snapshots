...
//	echo 'Day of week: '.date("w",$ts)."\n";
	...
	if (date("d",$ts)==1) { echo "1st of month\tKEEP\n" ; return true; }
	if ((date("w",$ts)==0) && $ts>$older_than_month) { echo "Recent Sunday\tKEEP\n" ; return true; } // What if too old?
	if ((date("w",$ts)==0) && $ts<=$older_than_month) { echo "Old Sunday\tDELETE\n" ; return false; } // What if too old?
	if ($ts>=$older_than) { echo "Recent backup\tKEEP\n" ; return true; } // What if too old?
	if ($ts<$older_than) { echo "Old backup\tDELETE\n" ; return false; } // What if too old?
